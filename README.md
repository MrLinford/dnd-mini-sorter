...
**Cause:** File move errors, or duplicate files moved into `Quarantine/` for review.

**Solution:**

* Check the log for duplicate and quarantine messages.
* A newer source replaces the destination, while the older destination is moved to `Quarantine/`.
* An older or unchanged source is moved to `Quarantine/`, while the destination remains in place.
